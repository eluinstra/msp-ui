...
import { serialPort1, serialPort2 } from '../serialport/serialport-driver';
...
const imuSens1 : ImuDataTypeSensor = {
  sensorname: "sensor1",
  timems: ""+currentTimeInMilliseconds,
  x: 0,
  y: 0,
  z: 0
};

const imuSens2 : ImuDataTypeSensor = {
  sensorname: "sensor2",
  timems: ""+currentTimeInMilliseconds,
  x: 0,
  y: 0,
...
//const [db, setDB] = useState<RxDatabase<ImuDatabaseCollections>>();
...
export const imuRequest1 = (cmd, payload) => {
  serialPort1?.value.write(Buffer.from(command(cmd, payload)))
}

export const imuRequest2 = (cmd, payload) => {
  serialPort2?.value.write(Buffer.from(command(cmd, payload)))
}

export const imuResponse1$ = new Subject<IImuMsgAcc>();
export const registerPortIMU1 = () => {
  serialPort1?.value.on('data', function (data) {
    let counter = 0
    for (let i = 0; i < data.length; i++) {
      //if 0x55 is found unpack messages till next 0x55
      parseIMUDataAcc("sensor1", data.readInt8(i))

      if (imuMsg.state == ImuState.IMU_COMMAND_RECEIVED) {
          imuResponse1$.next(imuMsgAcc)
          imuMsg.state = ImuState.IMU_IDLE
        } else if (imuMsg.state == ImuState.IMU_ERROR_RECEIVED) {
          imuResponse1$.error(new Error('MSP error received!'))
          imuMsg.state = ImuState.IMU_IDLE
        }
...
export const imuResponse2$ = new Subject<IImuMsgAcc>();
export const registerPortIMU2 = () => {
  serialPort2?.value.on('data', function (data) {
    let counter = 0
    for (let i = 0; i < data.length; i++) {
      //if 0x55 is found unpack messages till next 0x55
      parseIMUDataAcc("sensor2", data.readInt8(i))

      if (imuMsg.state == ImuState.IMU_COMMAND_RECEIVED) {
          imuResponse2$.next(imuMsgAcc)
          imuMsg.state = ImuState.IMU_IDLE
        } else if (imuMsg.state == ImuState.IMU_ERROR_RECEIVED) {
          imuResponse2$.error(new Error('MSP error received!'))
          imuMsg.state = ImuState.IMU_IDLE
        }
...
function parseIMUDataAcc(sensorname: string, num: Number) {
  switch (num) {
    case ImuState.IMU_PREFIX:
      if (!messageStarted)
        {
  ...
  if (imuMsgAcc.enable)
     {
      ...
      // the collecting of data must me async
      //addImuDatatoDB(Promise.resolve(db));
      // imuSens1.sensorname = sensorname;
      // imuSens1.time = currentTimeInMilliseconds;
      // imuSens1.x = ax;
      // imuSens1.y = ay;
      // imuSens1.z = az;

      var todo : ImuDataTypeSensor = {
        // _id: new Date().toISOString(),
        // title: 'sampleset_'+sensorname,
        // xa: ax,
        // ya: ay,
        // za: az,
        // completed: true
      ...
      //var jsonArrData = JSON.stringify(imuSens1);
      ...
      //console.log(">>> "+sensorname+"-"+currentTimeInMilliseconds+"\n" );
